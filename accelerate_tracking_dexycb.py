...
parser.add_argument('--filelist', type=str, default='./data/dexycb_filelist/val_videos.txt', help='path to the segmentation folder')
# set the gpu
...
parser.add_argument('--output_fps', type=int, default=15, help='Output video fps and total frames')
# crop the video
...
parser.add_argument('--bg_mv', action='store_true', help='whether to use background motion')
...
def process_video(args, model, MonoDEst_M, vid_name, root_dir):
    # Set output path
    device = next(model.parameters()).device
    vid_dir = os.path.join(root_dir, vid_name)
    vid_name_without_ext = vid_name.split('.')[0].split('/')[-1]
    outdir = os.path.dirname(vid_name).replace('dexycb_videos', 'dexycb_fore_tracking')
    ...
    # Remove bg
    if args.bg_mv:
        labels_dir = os.path.dirname(vid_dir).replace('dexycb_videos', 'dexycb')
        mask_seq_paths = [mask_seq_path for mask_seq_path in sorted(os.listdir(labels_dir), 
                    key=lambda x: int(x.split('.')[0].split('_')[-1])) if mask_seq_path.startswith('labels_')]
        for i in range(len(mask_seq_paths)):
            mask_seq_paths[i] = os.path.join(labels_dir, mask_seq_paths[i])
        
        mask_seq_paths = mask_seq_paths[:video.shape[1]]
        masks = []
        for mask_seq_path in mask_seq_paths:
            mask = np.load(mask_seq_path)['seg']
            mask[mask != 0] = 1
        ...
        for i in range(video.shape[1]):
            video[:, i] = video[:, i] * masks[i]    
        # video = torch.stack(new_video, dim=0)
    
    transform = transforms.Compose([
        transforms.Resize((int(384*args.crop_factor),
                                int(640*args.crop_factor))),  
    ])
...
if __name__ == "__main__":
    ...
    # Get all video files
    video_files = [f for f in open(args.filelist).read().splitlines() if f][:]
    ...
    for vid_name in video_files[start_idx:end_idx]:
        ...
        process_video(args, model, depth_preprocessor, vid_name, args.root)
