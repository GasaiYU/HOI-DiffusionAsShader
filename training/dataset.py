...
from training.hoi_utils import showHandJoints, CLASS_PROTOCAL, convert_gray_to_color
...
class HOIVideoDatasetResizing(VideoDataset):
    ...
    def _load_preprocessed_latents_and_embeds(
        ...
        if (tracking_latent_path is not None and tracking_image_latent_path is not None) and (random.random() < 0.8 or not self.random_mask):
        ...
        if (depth_latent_path is not None and depth_image_latent_path is not None) and (random.random() < 0.8 or not self.random_mask):
        ...
        if (seg_mask_latent_path is not None and seg_mask_image_latent_path is not None) and (random.random() < 0.8 or not self.random_mask):
        ...
        if (hand_keypoints_latent_path is not None and hand_keypoints_image_latent_path is not None) and (random.random() < 0.8 or not self.random_mask):
...
if __name__ == "__main__":
    ...
    rgb_video = (rgb_video.permute(0, 2, 3, 1).numpy() * 255).astype(np.uint8)
    tracking_video = (tracking_video.permute(0, 2, 3, 1).numpy() * 255).astype(np.uint8)
    depth_video = (depth_video.permute(0, 2, 3, 1).numpy() * 255).astype(np.uint8)
    normal_video = (normal_video.permute(0, 2, 3, 1).numpy() * 255).astype(np.uint8)
    seg_mask = (seg_mask.permute(0, 2, 3, 1).numpy() * 255).astype(np.uint8)
    hand_keypoints = (hand_keypoints.permute(0, 2, 3, 1).numpy() * 255).astype(np.uint8)    
