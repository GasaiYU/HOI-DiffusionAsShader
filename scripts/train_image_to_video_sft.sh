...
CAPTION_COLUMN="data/train_prompts.txt"
VIDEO_COLUMN="data/train_videos.txt"
TRACKING_COLUMN="data/train_trackings.txt"

# validation parameters
TRACKING_MAP_PATH="./val_resources/val_tracking.mp4"
...
VALIDATION_IMAGES="./val_resources/val_image.jpg"
  ...
  for lr_schedule in "${LR_SCHEDULES[@]}"; do
    for optimizer in "${OPTIMIZERS[@]}"; do
      for steps in "${MAX_TRAIN_STEPS[@]}"; do
        output_dir="./exps/cogshader_HOI_${steps}__optimizer_${optimizer}__lr-schedule_${lr_schedule}__learning-rate_${learning_rate}/"

        cmd="accelerate launch --config_file $ACCELERATE_CONFIG_FILE --gpu_ids $GPU_IDS --main_process_port $PORT training/cogvideox_image_to_video_sft.py \
